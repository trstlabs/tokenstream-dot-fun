...
export const transactionHistoryVersionAtom = atomWithStorage<
  number | undefined
>(LOCAL_STORAGE_KEYS.transactionHistoryVersion, undefined);
...
export const setTransactionHistoryAtom = atom(
  ...
  (
    get,
    set,
    historyItem: Partial<TransactionHistoryItem> &
      Pick<TransactionHistoryItem, "timestamp">
  ) => {
    const history = get(transactionHistoryAtom);

    const index = history.findIndex(
      (item) => item.timestamp === historyItem.timestamp
    );
...
export const lastTransactionInTimeAtom = atom((get) => {
  ...
  const originalIndex = history.findIndex(
    (tx) => tx.timestamp === lastTx.timestamp
  );
