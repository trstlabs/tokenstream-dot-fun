  ...
  return useQuery({
    ...
    queryFn: async ({ queryKey: [, txsRequired, txs] }) => {
      ...
      try {
        ...
        const updatedTransactionDetails: TransactionDetails[] = txs.map(
          (tx) => ({
            ...tx,
        ...
        const transferEvents =
          getTransferEventsFromTxStatusResponse(validStatuses);
        ...
        const lastTxStatus = lastTx?.state
          ? getSimpleOverallStatus(lastTx.state)
          : undefined;
        ...
        const resData: TxsStatus = {
          isSuccess:
            isAllSettled && !someTxFailed && lastTxStatus === "success",
          lastTxStatus,
